...
<html>
    <head>
        ...
        <!-- Title -->
        <title>Eyewear.lk  | Password Reset</title>
    ...
    <body class="page-login">
                    ...
                    <div class="row">
                        <div class="col-md-6 center">
                            <div class="login-box">
                                ...
                                <p class="text-center m-t-md"><?= $name ?>, Please enter new password.</p>
                                <?php
                                ...
                                ?>
                                <form class="m-t-md" action="<?= base_url('admin_action/update_password') ?>" method="post" id="password-update-form" >
                                    <input type="hidden" name="reset-code" id="reset-code" value="<?= $code ?>"/>
                                    <div class="form-group">
                                        <input type="password" class="form-control round-input" name="password1" id="password1" maxlength="20" placeholder="Password" required>
                                    </div>
                                    <div class="form-group">
                                        <input type="password" class="form-control round-input" name="password2" id="password2" maxlength="20" placeholder="Re-type password" required>
                                    </div>
                                    <button type="submit" class="btn btn-success btn-block round-input" >Update Password</button>
                                </form>
                            </div>
                        </div>
                    </div><!-- Row -->
        ...
        <script>
            ...
            messages[168] = '<?= $this->config->item(168, 'msg') ?>';
            messages[169] = '<?= $this->config->item(169, 'msg') ?>';
            $('#password-update-form').on('submit', function(e){
                e.preventDefault();
                e.stopPropagation();
                var pass_1 = $("#password1").val().trim();
                if (pass_1.length >= 8 && pass_1.length < 21) {
                    var pass_2 = $("#password2").val().trim();
                    if (pass_1 === pass_2) {
                        sed_ajax(BASEURL + 'admin_action/update_password', {'password':pass_1,'reset-code':$('#reset-code').val()}, 'json',on_update_success,on_update_fail);
                    } else {
                        show_message(152, messages[152], null, false, 'Password Update');
                    }
                } else {
                    show_message(168, messages[168], null, false, 'Password Update');
                }
                return false;
            });
        </script>
