...
class Order_model extends MY_Model {
    ...
    public function create_order($cart_items, $calculate_tax, $customer_id) {
        ...
        if ($this->insert_order($customer_id, $order_date)) {
            ...
            foreach ($cart_items as $item) {
                //Not allowed out of order
                if (ALLOW_ORDER_OUT_OF_STOCKS == FALSE) {
                    //compare stocks by grn
                    if (SALES_PROCESS_TYPE == 'POS') {
                        $item_qty = $item['qty'];
                        $grn_details = $this->Itemmodel->get_grn_items_by_item_id($item['item_data']['id']);
                        $nof_grn = 0;
                        while ($nof_grn < count($grn_details)) {
                            $balance = $grn_details[$nof_grn]['stock_in_hand'] - $item_qty;
                            if ($balance >= 0) {
                                $grn_stock_update = $this->Itemmodel->grn_stock_update($grn_details[$nof_grn]['id'], $balance);
                                if ($grn_stock_update) {
                                    $nof_grn = count($grn_details);
                                }
                            } else if ($balance == 0) {
                                $grn_stock_update = $this->Itemmodel->grn_stock_update($grn_details[$nof_grn]['id'], $balance);
                                $nof_grn = count($grn_details);
                                ...
                                $add_qty = 0;
                                $item_qty = preg_replace('[\D]', '', $balance);
                                $grn_stock_update = $this->Itemmodel->grn_stock_update($grn_details[$nof_grn]['id'], $add_qty);
                                $nof_grn++;
                            }
                        }
                    } else {
                        //sytem sales type Online
                        $item_qty = $item['qty'];
                        $grn_details = $this->Itemmodel->get_grn_items_by_item_id($item['item_data']['id']);
                        $nof_grn = 0;
                        while ($nof_grn < count($grn_details)) {
                            $balance = $grn_details[$nof_grn]['stock_in_hand'] - $item_qty;
                            if ($balance >= 0) {
                                $grn_stock_hold = $this->Itemmodel->grn_stock_hold($grn_details[$nof_grn]['id'], $item_qty);
                                if ($grn_stock_hold) {
                                    $nof_grn = count($grn_details);
                                }
                            } else if ($balance == 0) {
                                ...
                                $item_qty = preg_replace('[\D]', '', $balance);
                                $grn_stock_hold = $this->Itemmodel->grn_stock_hold($grn_details[$nof_grn]['id'], $grn_details[$nof_grn]['stock_in_hand']);
                                if ($grn_stock_hold) {
                                    $nof_grn++;
                                }
                            }
    ...
    private function insert_order($customer_id, $date) {
        ...
        return $this->db->insert('order', [
                    ...
                    'key' => $key,
                    'invoice_status' => INVOICE_STATUS_INVOICED,
                    'enabled' => 1,
    ...
    private $pending_orders_query = "SELECT `ord`.*, `cus`.`first_name`, `cus`.`last_name`, `cus`.`mobile_phone`, `cus`.`city`
FROM `order` `ord`
INNER JOIN `customer` `cus` ON `cus`.`id` = `ord`.`customer_id`
WHERE (`ord`.`order_status` = 'OPEN'
OR `ord`.`order_status` = 'ACCEPT'
OR `ord`.`order_status` = 'WORKING_PROGRESS')
AND ord.enabled = 1";
