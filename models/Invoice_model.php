...
class Invoice_model extends MY_Model {
    ...
    public function add_order_items_to_invoice($invoice_id, $cart) {
        ...
        $data = array(
            ...
            'item_id' => $cart['item'],
            'amount' => $cart['item_data']['price'] * $cart['qty'],
            'total_amount' => $cart['item_data']['price'] * $cart['qty'],
            'nof_items' => $cart['qty'],
        );
    ...
    public function get_invoice_details_by_id($invoice_id) {
        $this->db->select('oi.*,ord.customer_id,ord.order_number,ord.shipping_address_id,ord.order_date,ord.key as order_key');
    ...
    public function get_all_invoice() {
        ...
        for ($x = 0; $x < count($res); $x++) {
            ...
            $res[$x]['payment'] = $this->Payment_model->get_payments_by_invoice_id($res[$x]['id']);
        }

        return $res;
    }

    public function get_invoice_details_by_order_id($order_id) {
        $this->db->select('oi.*');
        $this->db->from('order_invoice oi');
        ...
        $res = $q->result_array();
        for ($x = 0; $x < count($res); $x++) {
            $res[$x]['payments'] = $this->Payment_model->get_payments_by_invoice_id($res[$x]['id']);
        }
        return $res;
    }
