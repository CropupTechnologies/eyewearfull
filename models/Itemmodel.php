...
//            $model_id = $values['model_id'];
    ...
    public function get_item_by_id($item_id, $holder_size = NULL) {
        ...
        if (count($res) > 0) {
            ...
            $item['stock'] = $this->item_avalibale_qty_by_item_id($item_id);
            return $item;
        }
        return FALSE;
    }
    ...
    public function get_all_grn_items_by_item_id($item_id) {
        $this->db->select('gi.*,g.date as grn_date');
        $this->db->from('grn_item gi');
        $this->db->join('grn g', 'g.id = gi.grn_id', 'inner');
        $this->db->where(array('gi.item_id' => $item_id, 'gi.enabled' => 1, 'g.enabled' => 1));
        $this->db->where('gi.stock_in_hand !=', 0);
    ...
    public function get_grn_items_by_item_id($item_id) {
        $res = array();
        $this->db->select('gi.*,g.date as grn_date');
        $this->db->from('grn_item gi');
        $this->db->join('grn g', 'g.id = gi.grn_id', 'inner');
        ...
        if (!empty($grns)) {
            foreach ($grns as $grn) {
                if ($grn['stock_in_hand'] > $grn['hold_qty']) {
                    array_push($res, $grn);
                }
            }
        }
    ...
    public function grn_stock_update($grn_item_id, $qty) {
        $data = array(
            'stock_in_hand' => $qty,
            'lastupdated_on' => date('Y-m-d H:i:s'),
            'lastupdated_by' => $_SESSION['customer']['id'],
        );
        $this->db->where('id', $grn_item_id);
        $res = $this->db->update('grn_item', $data);
        return $res;
    }

    public function grn_stock_hold($grn_item_id, $qty) {
        $grn_item = $this->get_grn_items_by_grn_item_id($grn_item_id);
        if (!empty($grn_item)) {
            $current_hold = $grn_item['hold_qty'];
        } else {
            $current_hold = 0;
        }
        $qty += $current_hold;
    ...
    public function get_grn_items_by_grn_item_id($grn_item_id) {
        $this->db->select('*');
    ...
    public function get_grn_history_by_item_id($item_id) {
        $q = "SELECT 
                    gi.*, g.date, g.ref_number, br.name as location_name,i.show_selling_price
                FROM
                    grn_item gi
                        JOIN
...
//                ->where('f.enabled', 1)
    ...
    function item_avalibale_qty_by_item_id($item_id) {
        $stock = $this->get_stock_qty_by_item($item_id);
        $hold = $this->get_hold_qty_by_item($item_id);
        $availble_qty = $stock - $hold;
        if ($availble_qty >= 0) {
            return $availble_qty;
        } else {
            return 0;
    ...
    public function get_stock_qty_by_item($item_id) {
        $this->db->select('sum(gi.stock_in_hand) as stock');
        $this->db->from('grn_item gi');
        $this->db->join('grn g', 'g.id = gi.grn_id', 'inner');
        $this->db->where(array('gi.item_id' => $item_id, 'gi.enabled' => 1, 'g.enabled' => 1));
        $this->db->where('gi.stock_in_hand !=', 0);
    ...
    public function get_hold_qty_by_item($item_id) {
        $this->db->select('sum(gi.hold_qty) as hold');
        $this->db->from('grn_item gi');
        $this->db->join('grn g', 'g.id = gi.grn_id', 'inner');
        $this->db->where(array('gi.item_id' => $item_id, 'gi.enabled' => 1, 'g.enabled' => 1));
        $this->db->where('gi.stock_in_hand !=', 0);
    ...
    public function get_random_itemsx() {
        $this->db->select('*');
        $this->db->from('item');
        $this->db->limit(15);
        $q = $this->db->get();
        ...
        if (!empty($res)) {
            for ($x = 0; $x < count($res); $x++) {
                $res[$x]['image'] = $this->get_single_item_image($res[$x]['id']);
                $res[$x]['price'] = $this->get_item_price($res[$x]['id']);
                $res[$x]['promotion'] = $this->get_item_promotion($res[$x]['id']);
            }
        }
        return $res;
    }

    public function get_random_itemsxxx() {
        $res = array();
        $x = 0;
        $items = $this->get_random_item_show_home_page();
        foreach ($items as $item) {
            //while ($x <= 11) {

            if ($item_price != null) {
                while ($x <= 11) {
                    $item_price = $this->get_item_price($item['id']);
                    $res[$x]['image'] = $this->get_single_item_image($item['id']);
                    $res[$x]['promotion'] = $this->get_item_promotion($item['id']);
                    $res[$x]['price'] = $this->get_item_price($item['id']);
    ...
    public function get_random_items() {
        ...
        $x = 0;
        $items = $this->get_random_item_show_home_page();
        while ($nof_items < count($items) && $x < 12) {
            $item_price = $this->get_item_price($items[$nof_items]['id']);
            if ($item_price != null) {
                array_push($res, $items[$nof_items]);
                $res[$x]['image'] = $this->get_single_item_image($items[$nof_items]['id']);
                $res[$x]['promotion'] = $this->get_item_promotion($items[$nof_items]['id']);
                $res[$x]['price'] = $this->get_item_price($items[$nof_items]['id']);
                $x++;
                $nof_items++;
            } else {
                //item price null
    ...
    public function get_random_item_show_home_page() {
        $this->db->select('*');
        $this->db->from('item');
    ...
    public function get_item_promotion($item_id) {
        $date = date('Y-m-d H:i:s');
        $q = $this->get_promotion_by_item_sql;
        $q = str_replace('@date', $date, $q);
        $q = str_replace('@server_tz', SERVER_TIME_ZONE, $q);
        $q = str_replace('@local_tz', LOCAL_TIME_ZONE, $q);
        $q = str_replace('@item_id', $item_id, $q);
        $res = $this->db->query($q)->result_array();
        if (!empty($res)) {
            return $res[0];
        } else {
...
//////////////// Query Section 
    private $get_promotion_by_item_sql = "
        SELECT 
    *
FROM
    promotion_items pi
        INNER JOIN
    promotion p ON p.id = pi.promotion_id
WHERE
    pi.start_date <= DATE_FORMAT(CONVERT_TZ('@date', '@server_tz', '@local_tz'),
            '%Y-%m-%d')
        AND pi.end_date >= DATE_FORMAT(CONVERT_TZ('@date', '@server_tz', '@local_tz'),
            '%Y-%m-%d')
